...
const useAuth = () => {
  ...
  const login = async (values, onError) => {
    try {
      ...
      // 2--STOCKAGE DANS LOCAL STORAGE de l'USER (de l'email + token) - pour securité:
      // bien de ne pas stocker mdp
      ...
      // locale storage accepte uniquement les chaînes de
      // caractère (JSON.str convertit objet en chaîne)
      localStorage.setItem("user", JSON.stringify(userData));

      // 3--MISE A JOUR DU CONTEXTE GLOBAL DE L'APPLICATION
      // on passe l'Auth Provider = on met à jour instantanément tous
      ...
      // on test que le 2e paramètre soit une fonction (on s'assure qu'il y
      // ait un 2e argument et qu'il soit une fonction)
  ...
  const logout = () => {
    ...
    // ici je reinitialise l'état global de l'application à
    // null (donc on enlève l'utilisateur de data)
    dispatch({
      payload: actions.HANDLE_USER,
      data: null,
    });
